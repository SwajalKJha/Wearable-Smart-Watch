...
void setup(void) {
  rtc.setTime(ss, mm, hh, 14, 05, 2024);  
  tft.init();
  tft.setRotation(0);
...
void loop() { //Watch face design & minutes-hour hand
  ...
  else if (rtc.getSecond() != lastsec)
  {
    ...
    else if (watchface == 3)
    {
      ...
      sdeg = rtc.getSecond() * 6;                
      mdeg = rtc.getMinute() * 6 + sdeg * 0.01666667; 
      hdeg = rtc.getHour() * 30 + mdeg * 0.0833333; 
