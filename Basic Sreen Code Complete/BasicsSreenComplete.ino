...
uint16_t osx = 120, osy = 140, omx = 120, omy = 140, ohx = 120, ohy = 140; 
...
String cc[12] = {"55", "50", "45", "40", "35", "30", "25", "20", "15", "10", "5", "0"};
...
void updateClock() {
  // Get current time from the NTP client
  ...
  // Draw clock on the TFT display
  drawClock(hh, mm, ss, dayOfMonth, dayOfWeek);
}

void drawClock(int hh, int mm, int ss, int dayOfMonth, int dayOfWeek) {
  // Your existing code for drawing the clock goes here
  // Remember to replace ss, mm, hh
  // with ss, mm, hh respectively
  ...
  else if (ss != lastsec)
  {
    if (watchface == 1 || watchface == 2)
    {
      ...
      if (hh > 10 && mm > 10)
      {
        tft.drawString(String(hh) + ":" + String(mm), 46, 136);
      }
      else if (hh < 10 && mm > 10)
      {
        tft.drawString("0" + String(hh) + ":" + String(mm), 46, 136);
      }
      else if (hh > 10 && mm < 10)
      {
        tft.drawString(String(hh) + ":0" + String(mm), 46, 136);
      }
      else
      {
        tft.drawString("0" + String(hh) + ":0" + String(mm), 46, 146);
      }
      tft.setFreeFont(&DSEG7_Classic_Bold_20);
      if (ss < 10)
      {
        tft.drawString("0" + String(ss), 154, 146);
      }
      else
      {
        tft.drawString(String(ss), 154, 146);
      }
      tft.setFreeFont(&DSEG14_Classic_Bold_18);
      tft.drawString(days1[dayOfWeek], 94, 106);
      tft.drawString(String(dayOfMonth), 156, 106);
    }
    else if (watchface == 3)
    {
